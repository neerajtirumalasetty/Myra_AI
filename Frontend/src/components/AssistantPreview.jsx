...
function AssistantPreview() {
  ...
  return (
    <div className='flex items-center justify-center px-3 sm:px-4 py-10 sm:py-14'>
        ...
        <div className={`relative w-[280px] h-[450px] sm:w-[330px] md:h-[550px] rounded-[32px] sm:rounded-[42px] overflow-hidden transition-all duration-500 ${current.bg} ${current.cardBorder} shadow-[0_20px_80px_rgba(0,0,0,0.28)]`}>
           ...
           <div className='absolute top-4 right-4 sm:top-5 sm:right-5 z-30 flex items-center gap-2'>
            <button onClick={()=>setTheme("dark")} className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-[#050816] border transition-all cursor-pointer ${theme === "dark"
                ? "border-purple-400 scale-110"
                :"border-white/20"
            }`}/>

            <button onClick={()=>setTheme("light")} className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white border transition-all cursor-pointer ${theme === "light"
                ? "border-blue-400 scale-110"
                :"border-gray-300"
            }`}/>

            <button onClick={()=>setTheme("glass")} className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-gradient-to-br from-white/80 to-white/20 border transition-all cursor-pointer ${theme === "glass"
                ? "border-white scale-110"
                :"border-white/20"
            }`}/>

            <button onClick={()=>setTheme("neon")} className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-gradient-to-r from-emerald-400 to-green-500 border transition-all cursor-pointer ${theme === "neon"
           ...
           <div className='relative z-20 flex flex-col items-center justify-between h-full px-5 py-6 sm:py-8'>

                <div className='relative mt-1'>
                    <div className={`absolute inset-0 scale-[2] rounded-full blur-[80px] bg-gradient-to-r ${current.orb} opacity-60`}/>

                    <div className={`relative w-24 h-24 sm:-28 sm:h-28 md:w-32 md:h-32 rounded-full bg-gradient-to-br ${current.orb} shadow-[0_0_120_px_rgba(255,255,255,0.15)] animate-pulse before:absolute before:inset-0 before:rounded-full before:bg-white/20 before:blur-xl`}/>
                </div>
                ...
                <div className='relative mb-1'>
                    ...
                    <button className={`relative w-12 h-12 sm:w-15 sm:h-15 md:w-18 md:h-18 rounded-full bg-gradient-to-br ${current.button} ${current.micGlow} flex items-center justify-center`}>
                        <CiMicrophoneOn className='text-[#00000b9]' size={40}/>
                    </button>
                </div>
